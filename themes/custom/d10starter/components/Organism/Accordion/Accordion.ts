  ...
  class Panel {
    ...
    open() {
      ...
      if (prefersReducedMotion.matches) {
        this.section.dispatchEvent(new Event('transitionend'));
      }
  ...
  for (const accordion of accordions) {
    ...
    */
    for (const el of accordion.children) {
      ...
      // maybe eventually add an option for this (create a landmark), but by default don't do it
      // section.setAttribute('aria-labelledby', buttonId);
      section.classList.add(
        /* tw */ 'aria-hidden:invisible',
        /* tw */ 'overflow-hidden',
...
if (!window.IS_STORYBOOK) setUpAccordions();
